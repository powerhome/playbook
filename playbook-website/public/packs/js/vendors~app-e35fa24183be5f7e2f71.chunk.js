...
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "IDLE_BLOCKER", function() { return IDLE_BLOCKER; });
...
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "UNSAFE_DEFERRED_SYMBOL", function() { return UNSAFE_DEFERRED_SYMBOL; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "UNSAFE_DeferredData", function() { return DeferredData; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "UNSAFE_ErrorResponseImpl", function() { return ErrorResponseImpl; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "UNSAFE_convertRouteMatchToUiMatch", function() { return convertRouteMatchToUiMatch; });
...
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "UNSAFE_invariant", function() { return invariant; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "UNSAFE_warning", function() { return warning; });
...
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "isDeferredData", function() { return isDeferredData; });
...
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "redirectDocument", function() { return redirectDocument; });
...
function createHashHistory(options) {
  ...
  function createHashLocation(window, globalHistory) {
    let _parsePath = parsePath(window.location.hash.substr(1)),
...
function explodeOptionalSegments(path) {
  ...
  let _segments = _toArray(segments),
    first = _segments[0],
...
function generatePath(originalPath, params) {
  ...
  const segments = path.split(/\/+/).map((segment, index, array) => {
    ...
    if (keyMatch) {
      const _keyMatch = _slicedToArray(keyMatch, 3),
        key = _keyMatch[1],
...
function resolvePath(to, fromPathname) {
  ...
  let _ref4 = typeof to === "string" ? parsePath(to) : to,
...
class DeferredData {
  constructor(data, responseInit) {
    ...
    this.data = Object.entries(data).reduce((acc, _ref) => {
      let _ref5 = _slicedToArray(_ref, 2),
        key = _ref5[0],
        value = _ref5[1];
  ...
  get unwrappedData() {
    ...
    return Object.entries(this.data).reduce((acc, _ref2) => {
      let _ref6 = _slicedToArray(_ref2, 2),
        key = _ref6[0],
        value = _ref6[1];
      return Object.assign(acc, {
...
function createRouter(init) {
  ...
  if (initialMatches == null) {
    ...
    let _getShortCircuitMatch = getShortCircuitMatches(dataRoutes),
  ...
  async function navigate(to, opts) {
    ...
    let _normalizeNavigateOpt = normalizeNavigateOptions(future.v7_normalizeFormMethod, false, normalizedPath, opts),
  ...
  async function startNavigation(historyAction, location, opts) {
    ...
    if (!matches) {
      ...
      let _getShortCircuitMatch2 = getShortCircuitMatches(routesToUse),
        notFoundMatches = _getShortCircuitMatch2.matches,
    ...
    // Call loaders
    let _await$handleLoaders = await handleLoaders(request, location, matches, loadingNavigation, opts && opts.submission, opts && opts.fetcherSubmission, opts && opts.replace, pendingActionData, pendingError),
      shortCircuited = _await$handleLoaders.shortCircuited,
      loaderData = _await$handleLoaders.loaderData,
      errors = _await$handleLoaders.errors;
    if (shortCircuited) {
      return;
    }
  ...
  async function handleLoaders(request, location, matches, overrideNavigation, submission, fetcherSubmission, replace, pendingActionData, pendingError) {
    ...
    let routesToUse = inFlightDataRoutes || dataRoutes;
    let _getMatchesToLoad = getMatchesToLoad(init.history, state, matches, activeSubmission, location, isRevalidationRequired, cancelledDeferredRoutes, cancelledFetcherLoads, fetchLoadMatches, fetchRedirectIds, routesToUse, basename, pendingActionData, pendingError),
      _getMatchesToLoad2 = _slicedToArray(_getMatchesToLoad, 2),
      matchesToLoad = _getMatchesToLoad2[0],
      revalidatingFetchers = _getMatchesToLoad2[1];
    ...
    let _await$callLoadersAnd = await callLoadersAndMaybeResolveData(state.matches, matches, matchesToLoad, revalidatingFetchers, request),
      results = _await$callLoadersAnd.results,
      loaderResults = _await$callLoadersAnd.loaderResults,
      fetcherResults = _await$callLoadersAnd.fetcherResults;
    if (request.signal.aborted) {
      return {
        shortCircuited: true
      };
    ...
    let _processLoaderData = processLoaderData(state, matches, matchesToLoad, loaderResults, pendingError, revalidatingFetchers, fetcherResults, activeDeferreds),
      loaderData = _processLoaderData.loaderData,
      errors = _processLoaderData.errors;
  ...
  function fetch(key, routeId, href, opts) {
    ...
    let _normalizeNavigateOpt2 = normalizeNavigateOptions(future.v7_normalizeFormMethod, true, normalizedPath, opts),
  ...
  async function handleFetcherAction(key, routeId, path, match, requestMatches, submission) {
    ...
    let _getMatchesToLoad3 = getMatchesToLoad(init.history, state, matches, submission, nextLocation, isRevalidationRequired, cancelledDeferredRoutes, cancelledFetcherLoads, fetchLoadMatches, fetchRedirectIds, routesToUse, basename, {
        [match.route.id]: actionResult.data
      }, undefined // No need to send through errors since we short circuit above
      ),
      _getMatchesToLoad4 = _slicedToArray(_getMatchesToLoad3, 2),
      matchesToLoad = _getMatchesToLoad4[0],
      revalidatingFetchers = _getMatchesToLoad4[1];
    ...
    let _await$callLoadersAnd2 = await callLoadersAndMaybeResolveData(state.matches, matches, matchesToLoad, revalidatingFetchers, revalidationRequest),
      results = _await$callLoadersAnd2.results,
      loaderResults = _await$callLoadersAnd2.loaderResults,
      fetcherResults = _await$callLoadersAnd2.fetcherResults;
    ...
    // Process and commit output from loaders
    let _processLoaderData2 = processLoaderData(state, state.matches, matchesToLoad, loaderResults, undefined, revalidatingFetchers, fetcherResults, activeDeferreds),
      loaderData = _processLoaderData2.loaderData,
      errors = _processLoaderData2.errors;
  ...
  async function startRedirectNavigation(state, redirect, _temp) {
    let _ref7 = _temp === void 0 ? {} : _temp,
      submission = _ref7.submission,
      fetcherSubmission = _ref7.fetcherSubmission,
    ...
    // state.navigation
    let _state$navigation = state.navigation,
  ...
  function abortStaleFetchLoads(landedId) {
    let yeetedKeys = [];
    for (let _ref8 of fetchReloadIds) {
  ...
  function shouldBlockNavigation(_ref2) {
    let currentLocation = _ref2.currentLocation,
    ...
    let _entries = _slicedToArray(entries[entries.length - 1], 2),
...
function createStaticHandler(routes, opts) {
  ...
  async function query(request, _temp2) {
    let _ref10 = _temp2 === void 0 ? {} : _temp2,
      requestContext = _ref10.requestContext;
    ...
    if (!isValidMethod(method) && method !== "HEAD") {
      ...
      let _getShortCircuitMatch3 = getShortCircuitMatches(dataRoutes),
        methodNotAllowedMatches = _getShortCircuitMatch3.matches,
        route = _getShortCircuitMatch3.route;
      return {
        basename,
        location,
      ...
      let _getShortCircuitMatch4 = getShortCircuitMatches(dataRoutes),
        notFoundMatches = _getShortCircuitMatch4.matches,
        route = _getShortCircuitMatch4.route;
      return {
        basename,
        location,
        matches: notFoundMatches,
  ...
  async function queryRoute(request, _temp3) {
    let _ref11 = _temp3 === void 0 ? {} : _temp3,
      routeId = _ref11.routeId,
      requestContext = _ref11.requestContext;
    let url = new URL(request.url);
    let method = request.method;
    let location = createLocation("", createPath(url), null, "default");
    let matches = matchRoutes(dataRoutes, location, basename);
...
function createClientSideRequest(history, location, signal, submission) {
  ...
  if (submission && isMutationMethod(submission.formMethod)) {
    let formMethod = submission.formMethod,
...
function convertFormDataToSearchParams(formData) {
  ...
  for (let _ref13 of formData.entries()) {
...
function convertSearchParamsToFormData(searchParams) {
  ...
  for (let _ref15 of searchParams.entries()) {
...
function processLoaderData(state, matches, matchesToLoad, results, pendingError, revalidatingFetchers, fetcherResults, activeDeferreds) {
  let _processRouteLoaderDa = processRouteLoaderData(matches, matchesToLoad, results, pendingError, activeDeferreds),
    loaderData = _processRouteLoaderDa.loaderData,
    errors = _processRouteLoaderDa.errors;
  // Process results from our revalidating fetchers
  for (let index = 0; index < revalidatingFetchers.length; index++) {
    let _revalidatingFetchers = revalidatingFetchers[index],
...
function getInternalRouterError(status, _temp4) {
  let _ref17 = _temp4 === void 0 ? {} : _temp4,
...
function getSubmissionFromNavigation(navigation) {
  let formMethod = navigation.formMethod,
...
/*!******************************************************!*\
  !*** ../node_modules/react-router-dom/dist/index.js ***!
  \******************************************************/
/*! exports provided: AbortedDeferredError, Await, MemoryRouter, Navigate, NavigationType, Outlet, Route, Router, RouterProvider, Routes, UNSAFE_DataRouterContext, UNSAFE_DataRouterStateContext, UNSAFE_LocationContext, UNSAFE_NavigationContext, UNSAFE_RouteContext, UNSAFE_useRouteId, createMemoryRouter, createPath, createRoutesFromChildren, createRoutesFromElements, defer, generatePath, isRouteErrorResponse, json, matchPath, matchRoutes, parsePath, redirect, redirectDocument, renderMatches, resolvePath, unstable_useBlocker, useActionData, useAsyncError, useAsyncValue, useHref, useInRouterContext, useLoaderData, useLocation, useMatch, useMatches, useNavigate, useNavigation, useNavigationType, useOutlet, useOutletContext, useParams, useResolvedPath, useRevalidator, useRouteError, useRouteLoaderData, useRoutes, BrowserRouter, Form, HashRouter, Link, NavLink, ScrollRestoration, UNSAFE_useScrollRestoration, createBrowserRouter, createHashRouter, createSearchParams, unstable_HistoryRouter, unstable_usePrompt, useBeforeUnload, useFetcher, useFetchers, useFormAction, useLinkClickHandler, useSearchParams, useSubmit */
/***/ (function(module, __webpack_exports__, __webpack_require__) {
...
/* harmony reexport (safe) */ __webpack_require__.d(__webpack_exports__, "UNSAFE_useRouteId", function() { return react_router__WEBPACK_IMPORTED_MODULE_1__["UNSAFE_useRouteId"]; });
...
/* harmony reexport (safe) */ __webpack_require__.d(__webpack_exports__, "redirectDocument", function() { return react_router__WEBPACK_IMPORTED_MODULE_1__["redirectDocument"]; });
...
/* harmony reexport (safe) */ __webpack_require__.d(__webpack_exports__, "unstable_useBlocker", function() { return react_router__WEBPACK_IMPORTED_MODULE_1__["unstable_useBlocker"]; });
...
function getFormEncType(encType) {
  if (encType != null && !supportedFormEncTypes.has(encType)) {
     true ? Object(_remix_run_router__WEBPACK_IMPORTED_MODULE_2__["UNSAFE_warning"])(false, "\"" + encType + "\" is not a valid `encType` for `<Form>`/`<fetcher.Form>` " + ("and will default to \"" + defaultEncType + "\"")) : undefined;
    return null;
  }
  return encType;
}
function getFormSubmissionInfo(target, basename) {
  ...
  if (isFormElement(target)) {
    ...
    action = attr ? Object(_remix_run_router__WEBPACK_IMPORTED_MODULE_2__["stripBasename"])(attr, basename) : null;
...
function BrowserRouter(_ref) {
  ...
  let _React$useState = react__WEBPACK_IMPORTED_MODULE_0__["useState"]({
...
function HashRouter(_ref2) {
  ...
  let history = historyRef.current;
  let _React$useState3 = react__WEBPACK_IMPORTED_MODULE_0__["useState"]({
      action: history.action,
      location: history.location
    }),
...
function HistoryRouter(_ref3) {
  let basename = _ref3.basename,
    children = _ref3.children,
    future = _ref3.future,
    history = _ref3.history;
  let _React$useState5 = react__WEBPACK_IMPORTED_MODULE_0__["useState"]({
      action: history.action,
      location: history.location
    }),
...
const Link = /*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_0__["forwardRef"](function LinkWithRef(_ref4, ref) {
  ...
  let _React$useContext = react__WEBPACK_IMPORTED_MODULE_0__["useContext"](react_router__WEBPACK_IMPORTED_MODULE_1__["UNSAFE_NavigationContext"]),
  ...
  if (typeof to === "string" && ABSOLUTE_URL_REGEX.test(to)) {
    ...
    if (isBrowser) {
      try {
        ...
        let path = Object(_remix_run_router__WEBPACK_IMPORTED_MODULE_2__["stripBasename"])(targetUrl.pathname, basename);
        ...
        // We can't do external URL detection without a valid URL
         true ? Object(_remix_run_router__WEBPACK_IMPORTED_MODULE_2__["UNSAFE_warning"])(false, "<Link to=\"" + to + "\"> contains an invalid URL which will probably break " + "when clicked - please update to a valid URL path.") : undefined;
      }
    }
  }
...
const NavLink = /*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_0__["forwardRef"](function NavLinkWithRef(_ref5, ref) {
  let _ref5$ariaCurrent = _ref5["aria-current"],
  ...
  let _React$useContext2 = react__WEBPACK_IMPORTED_MODULE_0__["useContext"](react_router__WEBPACK_IMPORTED_MODULE_1__["UNSAFE_NavigationContext"]),
...
const FormImpl = /*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_0__["forwardRef"]((_ref6, forwardedRef) => {
  let reloadDocument = _ref6.reloadDocument,
...
function useDataRouterContext(hookName) {
  ...
  !ctx ?  true ? Object(_remix_run_router__WEBPACK_IMPORTED_MODULE_2__["UNSAFE_invariant"])(false, getDataRouterConsoleError(hookName)) : undefined : void 0;
  return ctx;
}
function useDataRouterState(hookName) {
  ...
  !state ?  true ? Object(_remix_run_router__WEBPACK_IMPORTED_MODULE_2__["UNSAFE_invariant"])(false, getDataRouterConsoleError(hookName)) : undefined : void 0;
  return state;
}
/**
...
function useLinkClickHandler(to, _temp) {
  let _ref14 = _temp === void 0 ? {} : _temp,
    target = _ref14.target,
    replaceProp = _ref14.replace,
    state = _ref14.state,
    preventScrollReset = _ref14.preventScrollReset,
    relative = _ref14.relative;
...
function useSearchParams(defaultInit) {
   true ? Object(_remix_run_router__WEBPACK_IMPORTED_MODULE_2__["UNSAFE_warning"])(typeof URLSearchParams !== "undefined", "You cannot use the `useSearchParams` hook in a browser that does not " + "support the URLSearchParams API. If you need to support Internet " + "Explorer 11, we recommend you load a polyfill such as " + "https://github.com/ungap/url-search-params\n\n" + "If you're unsure how to load polyfills, we recommend you check out " + "https://polyfill.io/v3/ which provides some recommendations about how " + "to load polyfills only for users that need them, instead of for every " + "user.") : undefined;
 ...
 */
function useSubmit() {
  let _useDataRouterContext = useDataRouterContext(DataRouterHook.UseSubmit),
    router = _useDataRouterContext.router;
  let _React$useContext3 = react__WEBPACK_IMPORTED_MODULE_0__["useContext"](react_router__WEBPACK_IMPORTED_MODULE_1__["UNSAFE_NavigationContext"]),
    basename = _React$useContext3.basename;
  let currentRouteId = Object(react_router__WEBPACK_IMPORTED_MODULE_1__["UNSAFE_useRouteId"])();
  return react__WEBPACK_IMPORTED_MODULE_0__["useCallback"](function (target, options) {
    if (options === void 0) {
      options = {};
    }
    validateClientSideSubmission();
    let _getFormSubmissionInf = getFormSubmissionInfo(target, basename),
...
function useSubmitFetcher(fetcherKey, fetcherRouteId) {
  let _useDataRouterContext2 = useDataRouterContext(DataRouterHook.UseSubmitFetcher),
    router = _useDataRouterContext2.router;
  let _React$useContext4 = react__WEBPACK_IMPORTED_MODULE_0__["useContext"](react_router__WEBPACK_IMPORTED_MODULE_1__["UNSAFE_NavigationContext"]),
  ...
  return react__WEBPACK_IMPORTED_MODULE_0__["useCallback"](function (target, options) {
    ...
    validateClientSideSubmission();
    let _getFormSubmissionInf2 = getFormSubmissionInfo(target, basename),
    ...
    !(fetcherRouteId != null) ?  true ? Object(_remix_run_router__WEBPACK_IMPORTED_MODULE_2__["UNSAFE_invariant"])(false, "No routeId available for useFetcher()") : undefined : void 0;
...
function useFormAction(action, _temp2) {
  let _ref15 = _temp2 === void 0 ? {} : _temp2,
    relative = _ref15.relative;
  let _React$useContext5 = react__WEBPACK_IMPORTED_MODULE_0__["useContext"](react_router__WEBPACK_IMPORTED_MODULE_1__["UNSAFE_NavigationContext"]),
  ...
  !routeContext ?  true ? Object(_remix_run_router__WEBPACK_IMPORTED_MODULE_2__["UNSAFE_invariant"])(false, "useFormAction must be used inside a RouteContext") : undefined : void 0;
  let _routeContext$matches = routeContext.matches.slice(-1),
...
function useFetcher() {
  var _route$matches;
  let _useDataRouterContext3 = useDataRouterContext(DataRouterHook.UseFetcher),
  ...
  !route ?  true ? Object(_remix_run_router__WEBPACK_IMPORTED_MODULE_2__["UNSAFE_invariant"])(false, "useFetcher must be used inside a RouteContext") : undefined : void 0;
  let routeId = (_route$matches = route.matches[route.matches.length - 1]) == null ? void 0 : _route$matches.route.id;
  !(routeId != null) ?  true ? Object(_remix_run_router__WEBPACK_IMPORTED_MODULE_2__["UNSAFE_invariant"])(false, "useFetcher can only be used on routes that contain a unique \"id\"") : undefined : void 0;
  let _React$useState7 = react__WEBPACK_IMPORTED_MODULE_0__["useState"](() => String(++fetcherId)),
    _React$useState8 = _slicedToArray(_React$useState7, 1),
    fetcherKey = _React$useState8[0];
  let _React$useState9 = react__WEBPACK_IMPORTED_MODULE_0__["useState"](() => {
      !routeId ?  true ? Object(_remix_run_router__WEBPACK_IMPORTED_MODULE_2__["UNSAFE_invariant"])(false, "No routeId available for fetcher.Form()") : undefined : void 0;
      return createFetcherForm(fetcherKey, routeId);
    }),
    _React$useState10 = _slicedToArray(_React$useState9, 1),
    Form = _React$useState10[0];
  let _React$useState11 = react__WEBPACK_IMPORTED_MODULE_0__["useState"](() => href => {
      !router ?  true ? Object(_remix_run_router__WEBPACK_IMPORTED_MODULE_2__["UNSAFE_invariant"])(false, "No router available for fetcher.load()") : undefined : void 0;
      !routeId ?  true ? Object(_remix_run_router__WEBPACK_IMPORTED_MODULE_2__["UNSAFE_invariant"])(false, "No routeId available for fetcher.load()") : undefined : void 0;
...
function useScrollRestoration(_temp3) {
  let _ref16 = _temp3 === void 0 ? {} : _temp3,
    getKey = _ref16.getKey,
    storageKey = _ref16.storageKey;
  let _useDataRouterContext4 = useDataRouterContext(DataRouterHook.UseScrollRestoration),
    router = _useDataRouterContext4.router;
  let _useDataRouterState = useDataRouterState(DataRouterStateHook.UseScrollRestoration),
    restoreScrollPosition = _useDataRouterState.restoreScrollPosition,
    preventScrollReset = _useDataRouterState.preventScrollReset;
  let _React$useContext6 = react__WEBPACK_IMPORTED_MODULE_0__["useContext"](react_router__WEBPACK_IMPORTED_MODULE_1__["UNSAFE_NavigationContext"]),
    basename = _React$useContext6.basename;
  let location = Object(react_router__WEBPACK_IMPORTED_MODULE_1__["useLocation"])();
  let matches = Object(react_router__WEBPACK_IMPORTED_MODULE_1__["useMatches"])();
  let navigation = Object(react_router__WEBPACK_IMPORTED_MODULE_1__["useNavigation"])();
  ...
  if (typeof document !== "undefined") {
    ...
    react__WEBPACK_IMPORTED_MODULE_0__["useLayoutEffect"](() => {
      ...
      _extends({}, location, {
        pathname: Object(_remix_run_router__WEBPACK_IMPORTED_MODULE_2__["stripBasename"])(location.pathname, basename) || location.pathname
      }), matches) : getKey;
...
function usePrompt(_ref8) {
  ...
  let blocker = Object(react_router__WEBPACK_IMPORTED_MODULE_1__["unstable_useBlocker"])(when);
  react__WEBPACK_IMPORTED_MODULE_0__["useEffect"](() => {
...
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "UNSAFE_mapRouteProperties", function() { return mapRouteProperties; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "UNSAFE_useRouteId", function() { return useRouteId; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "UNSAFE_useRoutesImpl", function() { return useRoutesImpl; });
...
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "unstable_useBlocker", function() { return useBlocker; });
...
/* harmony reexport (safe) */ __webpack_require__.d(__webpack_exports__, "AbortedDeferredError", function() { return _remix_run_router__WEBPACK_IMPORTED_MODULE_1__["AbortedDeferredError"]; });

/* harmony reexport (safe) */ __webpack_require__.d(__webpack_exports__, "NavigationType", function() { return _remix_run_router__WEBPACK_IMPORTED_MODULE_1__["Action"]; });

/* harmony reexport (safe) */ __webpack_require__.d(__webpack_exports__, "createPath", function() { return _remix_run_router__WEBPACK_IMPORTED_MODULE_1__["createPath"]; });

/* harmony reexport (safe) */ __webpack_require__.d(__webpack_exports__, "defer", function() { return _remix_run_router__WEBPACK_IMPORTED_MODULE_1__["defer"]; });

/* harmony reexport (safe) */ __webpack_require__.d(__webpack_exports__, "generatePath", function() { return _remix_run_router__WEBPACK_IMPORTED_MODULE_1__["generatePath"]; });

/* harmony reexport (safe) */ __webpack_require__.d(__webpack_exports__, "isRouteErrorResponse", function() { return _remix_run_router__WEBPACK_IMPORTED_MODULE_1__["isRouteErrorResponse"]; });

/* harmony reexport (safe) */ __webpack_require__.d(__webpack_exports__, "json", function() { return _remix_run_router__WEBPACK_IMPORTED_MODULE_1__["json"]; });

/* harmony reexport (safe) */ __webpack_require__.d(__webpack_exports__, "matchPath", function() { return _remix_run_router__WEBPACK_IMPORTED_MODULE_1__["matchPath"]; });

/* harmony reexport (safe) */ __webpack_require__.d(__webpack_exports__, "matchRoutes", function() { return _remix_run_router__WEBPACK_IMPORTED_MODULE_1__["matchRoutes"]; });

/* harmony reexport (safe) */ __webpack_require__.d(__webpack_exports__, "parsePath", function() { return _remix_run_router__WEBPACK_IMPORTED_MODULE_1__["parsePath"]; });

/* harmony reexport (safe) */ __webpack_require__.d(__webpack_exports__, "redirect", function() { return _remix_run_router__WEBPACK_IMPORTED_MODULE_1__["redirect"]; });

/* harmony reexport (safe) */ __webpack_require__.d(__webpack_exports__, "redirectDocument", function() { return _remix_run_router__WEBPACK_IMPORTED_MODULE_1__["redirectDocument"]; });

/* harmony reexport (safe) */ __webpack_require__.d(__webpack_exports__, "resolvePath", function() { return _remix_run_router__WEBPACK_IMPORTED_MODULE_1__["resolvePath"]; });
...
function useHref(to, _temp) {
  ...
  !useInRouterContext() ?  true ? Object(_remix_run_router__WEBPACK_IMPORTED_MODULE_1__["UNSAFE_invariant"])(false,
  // TODO: This error is probably because they somehow have 2 versions of the
  // router loaded. We can help them understand how to avoid that.
  "useHref() may be used only in the context of a <Router> component.") : undefined : void 0;
  let _React$useContext = react__WEBPACK_IMPORTED_MODULE_0__["useContext"](NavigationContext),
    basename = _React$useContext.basename,
  ...
  if (basename !== "/") {
    joinedPathname = pathname === "/" ? basename : Object(_remix_run_router__WEBPACK_IMPORTED_MODULE_1__["joinPaths"])([basename, pathname]);
  }
...
function useMatch(pattern) {
  ...
  return react__WEBPACK_IMPORTED_MODULE_0__["useMemo"](() => Object(_remix_run_router__WEBPACK_IMPORTED_MODULE_1__["matchPath"])(pattern, pathname), [pathname, pattern]);
}
...
function useNavigate() {
  let _React$useContext2 = react__WEBPACK_IMPORTED_MODULE_0__["useContext"](RouteContext),
...
function useNavigateUnstable() {
  ...
  let _React$useContext3 = react__WEBPACK_IMPORTED_MODULE_0__["useContext"](NavigationContext),
    basename = _React$useContext3.basename,
    navigator = _React$useContext3.navigator;
  let _React$useContext4 = react__WEBPACK_IMPORTED_MODULE_0__["useContext"](RouteContext),
    matches = _React$useContext4.matches;
  let _useLocation2 = useLocation(),
    locationPathname = _useLocation2.pathname;
  let routePathnamesJson = JSON.stringify(Object(_remix_run_router__WEBPACK_IMPORTED_MODULE_1__["UNSAFE_getPathContributingMatches"])(matches).map(match => match.pathnameBase));
  ...
  let navigate = react__WEBPACK_IMPORTED_MODULE_0__["useCallback"](function (to, options) {
    if (options === void 0) {
     ...
     true ? Object(_remix_run_router__WEBPACK_IMPORTED_MODULE_1__["UNSAFE_warning"])(activeRef.current, navigateEffectWarning) : undefined;
    ...
    let path = Object(_remix_run_router__WEBPACK_IMPORTED_MODULE_1__["resolveTo"])(to, JSON.parse(routePathnamesJson), locationPathname, options.relative === "path");
    ...
    if (dataRouterContext == null && basename !== "/") {
      path.pathname = path.pathname === "/" ? basename : Object(_remix_run_router__WEBPACK_IMPORTED_MODULE_1__["joinPaths"])([basename, path.pathname]);
    }
 ...
 */
function useParams() {
  let _React$useContext5 = react__WEBPACK_IMPORTED_MODULE_0__["useContext"](RouteContext),
...
function useResolvedPath(to, _temp2) {
  ...
  let _React$useContext6 = react__WEBPACK_IMPORTED_MODULE_0__["useContext"](RouteContext),
    matches = _React$useContext6.matches;
  let _useLocation3 = useLocation(),
    locationPathname = _useLocation3.pathname;
  let routePathnamesJson = JSON.stringify(Object(_remix_run_router__WEBPACK_IMPORTED_MODULE_1__["UNSAFE_getPathContributingMatches"])(matches).map(match => match.pathnameBase));
  return react__WEBPACK_IMPORTED_MODULE_0__["useMemo"](() => Object(_remix_run_router__WEBPACK_IMPORTED_MODULE_1__["resolveTo"])(to, JSON.parse(routePathnamesJson), locationPathname, relative === "path"), [to, routePathnamesJson, locationPathname, relative]);
}
...
function useRoutesImpl(routes, locationArg, dataRouterState) {
  ...
  let _React$useContext7 = react__WEBPACK_IMPORTED_MODULE_0__["useContext"](NavigationContext),
    navigator = _React$useContext7.navigator;
  let _React$useContext8 = react__WEBPACK_IMPORTED_MODULE_0__["useContext"](RouteContext),
  ...
  if (locationArg) {
    ...
    let parsedLocationArg = typeof locationArg === "string" ? Object(_remix_run_router__WEBPACK_IMPORTED_MODULE_1__["parsePath"])(locationArg) : locationArg;
    !(parentPathnameBase === "/" || ((_parsedLocationArg$pa = parsedLocationArg.pathname) == null ? void 0 : _parsedLocationArg$pa.startsWith(parentPathnameBase))) ?  true ? Object(_remix_run_router__WEBPACK_IMPORTED_MODULE_1__["UNSAFE_invariant"])(false, "When overriding the location using `<Routes location>` or `useRoutes(routes, location)`, " + "the location pathname must begin with the portion of the URL pathname that was " + ("matched by all parent routes. The current pathname base is \"" + parentPathnameBase + "\" ") + ("but pathname \"" + parsedLocationArg.pathname + "\" was given in the `location` prop.")) : undefined : void 0;
  ...
  let matches = Object(_remix_run_router__WEBPACK_IMPORTED_MODULE_1__["matchRoutes"])(routes, {
    pathname: remainingPathname
  });
  if (true) {
     true ? Object(_remix_run_router__WEBPACK_IMPORTED_MODULE_1__["UNSAFE_warning"])(parentRoute || matches != null, "No routes matched location \"" + location.pathname + location.search + location.hash + "\" ") : undefined;
     true ? Object(_remix_run_router__WEBPACK_IMPORTED_MODULE_1__["UNSAFE_warning"])(matches == null || matches[matches.length - 1].route.element !== undefined || matches[matches.length - 1].route.Component !== undefined, "Matched leaf route at location \"" + location.pathname + location.search + location.hash + "\" " + "does not have an element or Component. This means it will render an <Outlet /> with a " + "null value by default resulting in an \"empty\" page.") : undefined;
  }
  let renderedMatches = _renderMatches(matches && matches.map(match => Object.assign({}, match, {
    params: Object.assign({}, parentParams, match.params),
    pathname: Object(_remix_run_router__WEBPACK_IMPORTED_MODULE_1__["joinPaths"])([parentPathnameBase,
    // Re-encode pathnames that were decoded inside matchRoutes
    navigator.encodeLocation ? navigator.encodeLocation(match.pathname).pathname : match.pathname]),
    pathnameBase: match.pathnameBase === "/" ? parentPathnameBase : Object(_remix_run_router__WEBPACK_IMPORTED_MODULE_1__["joinPaths"])([parentPathnameBase,
    // Re-encode pathnames that were decoded inside matchRoutes
...
function DefaultErrorComponent() {
  ...
  let message = Object(_remix_run_router__WEBPACK_IMPORTED_MODULE_1__["isRouteErrorResponse"])(error) ? error.status + " " + error.statusText : error instanceof Error ? error.message : JSON.stringify(error);
...
function _renderMatches(matches, parentMatches, dataRouterState) {
  ...
  if (errors != null) {
    ...
    !(errorIndex >= 0) ?  true ? Object(_remix_run_router__WEBPACK_IMPORTED_MODULE_1__["UNSAFE_invariant"])(false, "Could not find a matching route for errors on route IDs: " + Object.keys(errors).join(",")) : undefined : void 0;
...
function useDataRouterContext(hookName) {
  ...
  !ctx ?  true ? Object(_remix_run_router__WEBPACK_IMPORTED_MODULE_1__["UNSAFE_invariant"])(false, getDataRouterConsoleError(hookName)) : undefined : void 0;
  return ctx;
}
function useDataRouterState(hookName) {
  let state = react__WEBPACK_IMPORTED_MODULE_0__["useContext"](DataRouterStateContext);
  !state ?  true ? Object(_remix_run_router__WEBPACK_IMPORTED_MODULE_1__["UNSAFE_invariant"])(false, getDataRouterConsoleError(hookName)) : undefined : void 0;
  return state;
}
function useRouteContext(hookName) {
  let route = react__WEBPACK_IMPORTED_MODULE_0__["useContext"](RouteContext);
  !route ?  true ? Object(_remix_run_router__WEBPACK_IMPORTED_MODULE_1__["UNSAFE_invariant"])(false, getDataRouterConsoleError(hookName)) : undefined : void 0;
  return route;
}
...
function useCurrentRouteId(hookName) {
  ...
  !thisRoute.route.id ?  true ? Object(_remix_run_router__WEBPACK_IMPORTED_MODULE_1__["UNSAFE_invariant"])(false, hookName + " can only be used on routes that contain a unique \"id\"") : undefined : void 0;
 ...
 */
function useMatches() {
  let _useDataRouterState = useDataRouterState(DataRouterStateHook.UseMatches),
    matches = _useDataRouterState.matches,
    loaderData = _useDataRouterState.loaderData;
  return react__WEBPACK_IMPORTED_MODULE_0__["useMemo"](() => matches.map(m => Object(_remix_run_router__WEBPACK_IMPORTED_MODULE_1__["UNSAFE_convertRouteMatchToUiMatch"])(m, loaderData)), [matches, loaderData]);
}
...
function useActionData() {
  ...
  let route = react__WEBPACK_IMPORTED_MODULE_0__["useContext"](RouteContext);
  !route ?  true ? Object(_remix_run_router__WEBPACK_IMPORTED_MODULE_1__["UNSAFE_invariant"])(false, "useActionData must be used inside a RouteContext") : undefined : void 0;
...
function useBlocker(shouldBlock) {
  let _useDataRouterContext = useDataRouterContext(DataRouterHook.UseBlocker),
    router = _useDataRouterContext.router,
  ...
  let blockerFunction = react__WEBPACK_IMPORTED_MODULE_0__["useCallback"](arg => {
    ...
    // useLocation
    let currentLocation = arg.currentLocation,
      nextLocation = arg.nextLocation,
      historyAction = arg.historyAction;
    return shouldBlock({
      currentLocation: _extends({}, currentLocation, {
        pathname: Object(_remix_run_router__WEBPACK_IMPORTED_MODULE_1__["stripBasename"])(currentLocation.pathname, basename) || currentLocation.pathname
      }),
      nextLocation: _extends({}, nextLocation, {
        pathname: Object(_remix_run_router__WEBPACK_IMPORTED_MODULE_1__["stripBasename"])(nextLocation.pathname, basename) || nextLocation.pathname
      }),
      historyAction
    });
...
function useNavigateStable() {
  let _useDataRouterContext2 = useDataRouterContext(DataRouterHook.UseNavigateStable),
    router = _useDataRouterContext2.router;
...
function warningOnce(key, cond, message) {
  if (!cond && !alreadyWarned[key]) {
    alreadyWarned[key] = true;
     true ? Object(_remix_run_router__WEBPACK_IMPORTED_MODULE_1__["UNSAFE_warning"])(false, message) : undefined;
  }
}
...
function RouterProvider(_ref) {
  let fallbackElement = _ref.fallbackElement,
    router = _ref.router,
    future = _ref.future;
  // Need to use a layout effect here so we are subscribed early enough to
  // pick up on any render-driven redirects/navigations (useEffect/<Navigate>)
  let _React$useState3 = react__WEBPACK_IMPORTED_MODULE_0__["useState"](router.state),
    _React$useState4 = _slicedToArray(_React$useState3, 2),
    state = _React$useState4[0],
    setStateImpl = _React$useState4[1];
  let _ref11 = future || {},
...
function MemoryRouter(_ref3) {
  ...
  let historyRef = react__WEBPACK_IMPORTED_MODULE_0__["useRef"]();
  if (historyRef.current == null) {
    historyRef.current = Object(_remix_run_router__WEBPACK_IMPORTED_MODULE_1__["createMemoryHistory"])({
      initialEntries,
      initialIndex,
      v5Compat: true
    });
...
function Navigate(_ref4) {
  ...
  "<Navigate> may be used only in the context of a <Router> component.") : undefined : void 0;
   true ? Object(_remix_run_router__WEBPACK_IMPORTED_MODULE_1__["UNSAFE_warning"])(!react__WEBPACK_IMPORTED_MODULE_0__["useContext"](NavigationContext).static, "<Navigate> must not be used on the initial render in a <StaticRouter>. " + "This is a no-op, but you should modify your code so the <Navigate> is " + "only ever rendered in response to some user interaction or state change.") : undefined;
  let _React$useContext9 = react__WEBPACK_IMPORTED_MODULE_0__["useContext"](RouteContext),
    matches = _React$useContext9.matches;
  let _useLocation4 = useLocation(),
    locationPathname = _useLocation4.pathname;
  let navigate = useNavigate();

  // Resolve the path outside of the effect so that when effects run twice in
  // StrictMode they navigate to the same place
  let path = Object(_remix_run_router__WEBPACK_IMPORTED_MODULE_1__["resolveTo"])(to, Object(_remix_run_router__WEBPACK_IMPORTED_MODULE_1__["UNSAFE_getPathContributingMatches"])(matches).map(match => match.pathnameBase), locationPathname, relative === "path");
...
function Route(_props) {
   true ? Object(_remix_run_router__WEBPACK_IMPORTED_MODULE_1__["UNSAFE_invariant"])(false, "A <Route> is only ever to be used as the child of <Routes> element, " + "never rendered directly. Please wrap your <Route> in a <Routes>.") : undefined;
}
/**
...
function Router(_ref5) {
  ...
  !!useInRouterContext() ?  true ? Object(_remix_run_router__WEBPACK_IMPORTED_MODULE_1__["UNSAFE_invariant"])(false, "You cannot render a <Router> inside another <Router>." + " You should never have more than one in your app.") : undefined : void 0;
  ...
  if (typeof locationProp === "string") {
    locationProp = Object(_remix_run_router__WEBPACK_IMPORTED_MODULE_1__["parsePath"])(locationProp);
  }
  ...
  let locationContext = react__WEBPACK_IMPORTED_MODULE_0__["useMemo"](() => {
    let trailingPathname = Object(_remix_run_router__WEBPACK_IMPORTED_MODULE_1__["stripBasename"])(pathname, basename);
   ...
   true ? Object(_remix_run_router__WEBPACK_IMPORTED_MODULE_1__["UNSAFE_warning"])(locationContext != null, "<Router basename=\"" + basename + "\"> is not able to match the URL " + ("\"" + pathname + search + hash + "\" because it does not start with the ") + "basename, so the <Router> won't render anything.") : undefined;
...
function createRoutesFromChildren(children, parentPath) {
  ...
  react__WEBPACK_IMPORTED_MODULE_0__["Children"].forEach(children, (element, index) => {
    ...
    !(element.type === Route) ?  true ? Object(_remix_run_router__WEBPACK_IMPORTED_MODULE_1__["UNSAFE_invariant"])(false, "[" + (typeof element.type === "string" ? element.type : element.type.name) + "] is not a <Route> component. All component children of <Routes> must be a <Route> or <React.Fragment>") : undefined : void 0;
    !(!element.props.index || !element.props.children) ?  true ? Object(_remix_run_router__WEBPACK_IMPORTED_MODULE_1__["UNSAFE_invariant"])(false, "An index route cannot have child routes.") : undefined : void 0;
    let route = {
...
function mapRouteProperties(route) {
  ...
  if (route.Component) {
    if (true) {
      if (route.element) {
         true ? Object(_remix_run_router__WEBPACK_IMPORTED_MODULE_1__["UNSAFE_warning"])(false, "You should not include both `Component` and `element` on your route - " + "`Component` will be used.") : undefined;
      }
    }
  ...
  if (route.ErrorBoundary) {
    if (true) {
      if (route.errorElement) {
         true ? Object(_remix_run_router__WEBPACK_IMPORTED_MODULE_1__["UNSAFE_warning"])(false, "You should not include both `ErrorBoundary` and `errorElement` on your route - " + "`ErrorBoundary` will be used.") : undefined;
      }
    }
    Object.assign(updates, {
...
function createMemoryRouter(routes, opts) {
  return Object(_remix_run_router__WEBPACK_IMPORTED_MODULE_1__["createRouter"])({
    basename: opts == null ? void 0 : opts.basename,
    future: _extends({}, opts == null ? void 0 : opts.future, {
      v7_prependBasename: true
    }),
    history: Object(_remix_run_router__WEBPACK_IMPORTED_MODULE_1__["createMemoryHistory"])({
...
//# sourceMappingURL=vendors~app-e35fa24183be5f7e2f71.chunk.js.map