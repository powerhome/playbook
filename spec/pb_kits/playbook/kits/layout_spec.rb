...
RSpec.describe Playbook::PbLayout::Layout do
  ...
  describe "#classname" do
    it "returns namespaced class name", :aggregate_failures do
      text = "Test text"
      expect(subject.new({}).classname).to eq "pb_layout_kit_size_md_left_light layout_left_collapse_xs"
      expect(subject.new(full: true).classname).to eq "pb_layout_kit_size_md_left_light full layout_left_collapse_xs"
      expect(subject.new(variant: "dark", transparent: true, full: true).classname).to eq "pb_layout_kit_size_md_left_dark_transparent full layout_left_collapse_xs"
      expect(subject.new(variant: "dark", position: "right", size: "sm").classname).to eq "pb_layout_kit_size_sm_right_dark layout_right_collapse_xs"
    end
  end
end
