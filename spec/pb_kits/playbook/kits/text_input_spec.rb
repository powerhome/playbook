...
RSpec.describe Playbook::PbTextInput::TextInput do
  ...
  describe "#classname" do
    it "returns namespaced class name", :aggregate_failures do
      ...
      expect(subject.new({error:true, error_message: "Please enter a valid email"}).classname).to eq "pb_text_input_kit error"
      expect(subject.new({dark: true, error:true}).classname).to eq "pb_text_input_kit dark error"
    end
  end
end
