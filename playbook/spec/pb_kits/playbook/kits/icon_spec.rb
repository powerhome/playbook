...
RSpec.describe Playbook::PbIcon::Icon do
  ...
  describe "#classname" do
    it "returns namespaced class name", :aggregate_failures do
      ...
      expect(subject.new(icon: icon, border: true, fixed_width: false).classname).to eq "pb_icon_kit fas fa-user fa-border"
      expect(subject.new(icon: icon).classname).to eq "pb_icon_kit fas fa-user fa-fw"
      expect(subject.new(icon: icon, flip: "horizontal").classname).to eq "pb_icon_kit fas fa-user fa-fw fa-flip-horizontal"
      expect(subject.new(icon: icon).classname).to eq "pb_icon_kit fas fa-user fa-fw"
      expect(subject.new(icon: icon, inverse: true).classname).to eq "pb_icon_kit fas fa-user fa-fw fa-inverse"
      expect(subject.new(icon: icon, list_item: true).classname).to eq "pb_icon_kit fas fa-user fa-fw fa-li"
      expect(subject.new(icon: icon, pull: pull).classname).to eq "pb_icon_kit fas fa-user fa-fw fa-pull-#{pull}"
      expect(subject.new(icon: icon, pulse: true).classname).to eq "pb_icon_kit fas fa-user fa-fw fa-pulse"
      expect(subject.new(icon: icon, rotation: rotation).classname).to eq "pb_icon_kit fas fa-user fa-fw fa-rotate-#{rotation}"
      expect(subject.new(icon: icon, size: size).classname).to eq "pb_icon_kit fas fa-user fa-fw fa-#{size}"
      expect(subject.new(icon: icon, spin: true).classname).to eq "pb_icon_kit fas fa-user fa-fw fa-spin"
      expect(subject.new(icon: icon, classname: "additional_class").classname).to eq "pb_icon_kit fas fa-user fa-fw additional_class"
    end
