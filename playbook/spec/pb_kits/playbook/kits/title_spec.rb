...
RSpec.describe Playbook::PbTitle::Title do
  ...
  describe "#classname" do
    it "returns namespaced class name", :aggregate_failures do
      expect(subject.new({}).classname).to eq "pb_title_kit__3"

      expect(subject.new(classname: "additional_class").classname).to eq "pb_title_kit__3 additional_class"
      expect(subject.new(dark: true).classname).to eq "pb_title_kit__3 dark"
      expect(subject.new(size: nil).classname).to eq "pb_title_kit__3"
      expect(subject.new(size: 4).classname).to eq "pb_title_kit__4"
