...
test("renders default class name and percentage", () => {
  render(
    <div>
      <ProgressSimple data={{ testid: testId }} percent={45} />
    </div>
  )
...
test("renders values", () => {
  render(
    <div>
      <ProgressSimple data={{ testid: testId }} max="10" value="2" />
    </div>
  )
...
test("renders progress bar width", () => {
  render(
    <div>
      <ProgressSimple data={{ testid: testId }} percentage={40} width="100px" />
    </div>
  )
...
test("renders color variants", () => {
  ;["positive", "negative", "warning"].forEach(colorVariant => {
    render(
      <div>
        <ProgressSimple
          data={{ testid: testId }}
    ...
    const kit = screen.getByTestId(testId)
    const progress = kit.querySelector(
      `.pb_progress_simple_kit_${colorVariant}`
    )
...
test("renders muted prop", () => {
  render(
    <div>
      <ProgressSimple data={{ testid: testId }} muted percentage={40} />
    </div>
  )
...
test("renders align prop", () => {
  ;["left", "center", "right"].forEach(alignProp => {
    render(
      <ProgressSimple
        align={alignProp}
        data={{ testid: testId }}
