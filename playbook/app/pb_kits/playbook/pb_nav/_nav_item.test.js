...
const itemImageUrl =
  "https://upload.wikimedia.org/wikipedia/commons/0/00/Apple_News_icon_%28macOS%29.png"
...
test("should not have a left border", () => {
  ...
  expect(kit).toContainHTML(
    "pb_nav_list_kit_item_active_highlighted_border_none"
  )
})
...
test("should have a right icon", () => {
  ...
  const kit = screen.getByTestId(itemTestId)
  expect(kit).toContainHTML(
    '<i class="pb_icon_kit far fa-fw fa-angle-down pb_nav_list_item_icon_right" />'
  )
})
...
test("should have a left icon", () => {
  ...
  const kit = screen.getByTestId(itemTestId)
  expect(kit).toContainHTML(
    '<i class="pb_icon_kit far fa-fw fa-users-class pb_nav_list_item_icon_left" />'
  )
})
