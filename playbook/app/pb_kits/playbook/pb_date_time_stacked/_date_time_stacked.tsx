...
const DateTimeStacked = (props: DateTimeStackedProps): React.ReactElement => {
  ...
  const { date, datetime, dark, timeZone = "America/New_York" } = props

  const classes = buildCss("pb_date_time_stacked_kit", globalProps(props))

  return (
    <Flex inline={false} vertical="stretch" {...props}>
      <FlexItem>
        <DateStacked align="right" bold dark={dark} date={date || datetime} />
      </FlexItem>

      <SectionSeparator className="date-time-padding" orientation="vertical" />
      <FlexItem>
        <TimeStacked
          className={classes}
          dark={dark}
