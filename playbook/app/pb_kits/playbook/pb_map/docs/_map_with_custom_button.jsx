...
const MapWithCustomButton = props => {
  ...
  const defaultPosition = [-75.379143, 39.8312]

  // linking Maplibre methods to PB custom zoom in, zoom out, and fly to buttons
  const handleZoomIn = map => {
    map.zoomIn({ ...mapTheme.zoomConfig })
  }
  const handleZoomOut = map => {
    map.zoomOut({ ...mapTheme.zoomConfig })
  }
  const handleFlyTo = map => {
    map.flyTo({
    ...
    new maplibregl.Marker({
      ...
      .setPopup(
        new maplibregl.Popup({ closeButton: false }).setHTML(
          `<h4 class="pb_title_kit_size_4">Hello World!</h4>`
        )
      ) // add popup
      .addTo(map)
  ...
  return (
    <Map {...props}>
      <Map.Controls
        flyTo
        flyToClick={() => {
          handleFlyTo(mapInstance)
        }}
        zoomBtns
        zoomInClick={() => {
          handleZoomIn(mapInstance)
        }}
        zoomOutClick={() => {
          handleZoomOut(mapInstance)
        }}
      >
        <MapCustomButton icon="home" onClick={() => alert("button clicked!")} />
        <MapCustomButton
          icon="search"
