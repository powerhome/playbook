...
const MultiLevelSelectOptions = ({ children, items, ...props }: MultiLevelSelectOptionsProps) => {
  const {
    variant,
    inputName,
    renderNestedOptions,
  ...
  const { aria = {}, className, data = {}, htmlOptions = {} } = props
  ...
  return (
    <ul {...ariaProps} {...dataProps} {...htmlProps} className={classes}>
      {Array.isArray(items) &&
              ...
              <li className={"dropdown_item"} data-name={item.id}>
                <div className="dropdown_item_checkbox_row">
                  {!item.parent_id && !item.children ? null : (
                    <div key={isTreeRowExpanded(item) ? "chevron-down" : "chevron-right"}>
                      <CircleIconButton
                        className={item.children && item.children.length > 0 ? "" : "toggle_icon"}
                        icon={isTreeRowExpanded(item) ? "chevron-down" : "chevron-right"}
                        onClick={(event: React.MouseEvent) => handleToggleClick(item.id, event)}
                        variant="link"
                      />
                    </div>
                  )}
