...
const MultiLevelSelect = forwardRef<HTMLInputElement, MultiLevelSelectProps>((props) => {
  ...
  const addCheckedAndParentProperty = (
    ...
    return treeData.map((item: { [key: string]: any } | any) => {
      ...
      if (newItem.children && newItem.children.length > 0) {
        const children =
          item.checked && !returnAllSelected ? modifyRecursive(item.children, true) : item.children
  ...
  const modifyValue = (id: string, tree: { [key: string]: any }[], check: boolean) => {
    if (!Array.isArray(tree)) {
      return
    }
  ...
  const handleInputWrapperClick = (e: any) => {
    if (e.target.id === labelForId || e.target.classList.contains("pb_form_pill_tag") || disabled) {
      return
    }
  ...
  const handleToggleClick = (id: string, event: React.MouseEvent) => {
    ...
    if (clickedItem) {
      ...
      if (itemExpanded) expandedArray = expandedArray.filter((i) => i != clickedItem[0].id)
  ...
  return (
    <div {...ariaProps} {...dataProps} {...htmlProps} className={classes} id={id}>
      {label && (
        <label htmlFor={labelForId} onClick={handleLabelClick}>
          {requiredIndicator ? (
            <Caption className="pb_multi_level_select_kit_label" marginBottom="xs">
              {label} <span style={{ color: `${colors.error}` }}>*</span>
            </Caption>
          ) : (
            <Caption className="pb_multi_level_select_kit_label" marginBottom="xs" text={label} />
          )}
        </label>
      )}
      ...
      >
        <div className="wrapper" ref={dropdownRef}>
          <div className="input_wrapper" onClick={handleInputWrapperClick}>
            <div className="input_inner_container">
              ...
              {variant !== "single" && (
                <>
                  ...
                  {returnAllSelected && returnedArray.length !== 0 && inputDisplay === "pills"
            ...
            {isDropdownClosed ? (
              <div id={arrowDownElementId} key="chevron-down">
                <Icon icon="chevron-down" id={arrowDownElementId} size="xs" />
              </div>
            ) : (
              <div id={arrowUpElementId} key="chevron-up">
                <Icon icon="chevron-up" id={arrowUpElementId} size="xs" />
              </div>
            )}
          </div>
