...
test("returns namespaced class name", () => {
  render(<Passphrase data={{ testid: testId }} />)
...
test("returns additional class name", () => {
  render(<Passphrase className="additional_class" data={{ testid: testId }} />)
...
test("returns dark class name", () => {
  render(<Passphrase dark data={{ testid: testId }} />)
...
test("popover target shows when tips are given", () => {
  render(<Passphrase data={{ testid: testId }} tips={["some helpful tips"]} />)

  const kit = screen.getByTestId(testId)
  expect(
    kit.querySelector("[class^=pb_popover_reference_wrapper]")
  ).toBeDefined()
})
