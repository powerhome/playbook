...
test("subtitle prop adds subtitle text", () => {
  render(<User data={{ testid: "test-subtitle" }} subtitle="test subtitle" />)
...
test("subtitle prop accepts a node", () => {
  ...
  render(
    <User data={{ testid: "test-subtitle-block" }} subtitle={TestCaption} />
  )

  expect(screen.getByTestId("test-subtitle-block")).toHaveTextContent(
    "test caption"
  )
})
