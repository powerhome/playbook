...
test("should pass in url property and allow links to open in a new window", () => {
  ...
  expect(
    container.getElementsByClassName("pb_hashtag_kit")[0].firstChild
...
test("renders the badge kit with appropriate text", () => {
  ...
  expect(container.getElementsByClassName("pb_badge_kit_primary")[0])
    .toBeInTheDocument
  expect(
    container.getElementsByClassName("pb_badge_kit_primary")[0]
  ).toHaveTextContent(typeMap[type] + text)
