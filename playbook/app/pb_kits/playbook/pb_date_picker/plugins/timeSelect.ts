  ...
  return function (fp: FpTypes & any) {
    ...
    const updateMeridiemToggle = (forceClick: boolean) => {
      ...
      const uncheckedClass = "pb_selectable_card_kit_enabled",
        ...
        pickerAM: HTMLElement & { [x: string]: any } = document.getElementById(
          `datePicker-${fp.element.id}-AM`
        ),
        pickerPM: HTMLElement & { [x: string]: any } = document.getElementById(
          `datePicker-${fp.element.id}-PM`
        ),
