...
let activeLabel = "";
const quickPickPlugin = (thisRangesEndToday, customQuickPickDates) => {
    return function (fp) {
        ...
        const calculateDateRange = (timePeriod, amount) => {
            ...
            if (customQuickPickDates.dates.length && customQuickPickDates.override === false) {
                customQuickPickDates.dates.forEach((item) => {
                    if (Array.isArray(item.value)) {
                        ...
                        ranges[item.label] = calculateDateRange(item.value.timePeriod, item.value.amount);
                    }
                });
            }
        ...
        const isLabelMatchingSelectedDates = (selectedDates) => {
        ...
        return {
            ...
            onClose(selectedDates) {
                ...
                if (!isLabelMatchingSelectedDates(selectedDates)) {
                    (_a = pluginData.rangesButtons[activeLabel]) === null || _a === void 0 ? void 0 : _a.classList.remove('active');
                    activeLabel = "";
                }
