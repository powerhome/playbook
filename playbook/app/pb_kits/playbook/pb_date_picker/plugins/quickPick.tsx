...
const quickPickPlugin = (thisRangesEndToday: boolean) => {
  return function (fp: FpTypes & any): any {
    ...
    const thisWeekEndDate = thisRangesEndToday
      ? new Date()
      : DateTime.getLastDayOfWeek(new Date())
    ...
    const thisMonthEndDate = thisRangesEndToday
      ? new Date()
      : DateTime.getMonthEndDate(new Date())
    ...
    const thisQuarterEndDate = thisRangesEndToday
      ? new Date()
      : DateTime.getQuarterEndDate(new Date())
    const lastQuarterStartDate = DateTime.getPreviousQuarterStartDate(
      new Date()
    )
    ...
    const thisYearEndDate = thisRangesEndToday
      ? new Date()
      : DateTime.getYearEndDate(new Date())
    ...
    const isLabelMatchingSelectedDates = (selectedDates: Array<Date>) => {
      return (
        ...
        selectedDates[0].toDateString() ===
          pluginData.ranges[activeLabel][0].toDateString() &&
        selectedDates[1].toDateString() ===
          pluginData.ranges[activeLabel][1].toDateString()
      )
    }
    ...
    return {
      ...
      onClose(selectedDates: Array<Date>) {
        ...
        if (selectedDates.length < 2 && selectedDates.length > 0) {
          fp.input.placeholder = fp.formatDate(
