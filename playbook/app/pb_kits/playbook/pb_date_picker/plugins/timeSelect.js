...
function timeSelectPlugin(props) {
    return function (fp) {
        const generateMeridiemCard = (text) => {
            ...
            const cardInput = document.createElement("input"), cardInputId = `datePicker-${fp.element.id}-${text}`;
            ...
            const cardLabel = document.createElement("label"), cardLabelBuffer = document.createElement("div");
        ...
        const updateMeridiemToggle = (forceClick) => {
            if (!fp.selectedDates.length)
                return;
            const uncheckedClass = "pb_selectable_card_kit_enabled", checkedClass = "pb_selectable_card_kit_checked_enabled", pickerAM = document.getElementById(`datePicker-${fp.element.id}-AM`), pickerPM = document.getElementById(`datePicker-${fp.element.id}-PM`), meridiem = getMeridiem(fp.selectedDates);
            if (forceClick) {
        ...
        return {
            ...
            onReady() {
                ...
                if (!id || !(fp === null || fp === void 0 ? void 0 : fp.timeContainer))
                    return;
                ...
                if (props.caption) {
                    ...
                    captionContainer.innerHTML = props === null || props === void 0 ? void 0 : props.caption;
