...
const datePickerHelper = (
  ...
  const disabledParser = () => {
    ...
    disableRange &&
      disableRange.length > 0 &&
      disabledArray.push(...disableRange)
    disableWeekdays &&
      disableWeekdays.length > 0 &&
      disabledArray.push(...disabledWeekDays())
  ...
  const setPlugins = (thisRangesEndToday: boolean) => {
    ...
    if (enableTime)
      pluginList.push(
        timeSelectPlugin({ caption: timeCaption, showTimezone: showTimezone })
      )
    ...
    onClose: [
      (selectedDates, dateStr) => {
        ...
        if (!staticPosition && scrollContainer)
          detachFromScroll(scrollContainer as HTMLElement)
  ...
  const picker = document.querySelector<HTMLElement & { [x: string]: any }>(
    `#${pickerId}`
  )._flatpickr
  ...
  function attachToScroll(scrollParent: string | HTMLElement) {
    document
      .querySelectorAll(scrollParent as string)[0]
      ?.addEventListener("scroll", scrollEvent, { passive: true })
  }
  function detachFromScroll(
    scrollParent: string | HTMLElement = document.body
  ) {
    document
      .querySelectorAll(scrollParent as string)[0]
  ...
  const dropdown = document.querySelector<HTMLElement & { [x: string]: any }>(
    `#year-${pickerId}`
  )
  ...
  dropdown.insertAdjacentHTML(
    "afterend",
    '<i class="far fa-angle-down year-dropdown-icon" id="test-id"></i>'
  )
  if (picker.monthElements[0].parentElement) {
    return picker.monthElements[0].insertAdjacentHTML(
      "afterend",
      '<i class="far fa-angle-down month-dropdown-icon"></i>'
    )
  }
  ...
  document
    ...
    .parentElement.addEventListener("click", e => e.stopPropagation())
}
