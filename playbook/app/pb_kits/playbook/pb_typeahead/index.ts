...
export default class PbTypeahead extends PbEnhancedElement {
  ...
  connect() {
    ...
    this.resultsElement.addEventListener("click", (event: MouseEvent) =>
  ...
  search() {
    ...
    this.element.dispatchEvent(
      new CustomEvent("pb-typeahead-kit-search", {
        bubbles: true,
        detail: search,
      })
    )
  ...
  resultsCacheUpdate(
    ...
    if (this.resultsOptionCache.size > 32)
      this.resultsOptionCache.delete(
        this.resultsOptionCache.keys().next().value
      )
  ...
  showResults() {
    ...
    this.clearResults()
    for (const result of this.resultsOptionCache.get(
      this.searchTermAndContext
    )) {
      this.resultsElement.appendChild(
        this.newResultOption(result.cloneNode(true))
      )
    }
    for (const result of this.resultsElement.querySelectorAll(
      "[data-result-option-item]"
    )) {
  ...
  optionSelected(event: MouseEvent) {
    const resultOption = (event.target as Element).closest(
      "[data-result-option-item]"
    )
    ...
    this.element.dispatchEvent(
      new CustomEvent("pb-typeahead-kit-result-option-selected", {
        bubbles: true,
  ...
  newResultOption(content: DocumentFragment) {
    const resultOption = (
      this.resultOptionTemplate as HTMLTemplateElement
  ...
  focusPreviousOption() {
    const currentIndex = this.resultOptionItems.indexOf(
      this.currentSelectedResultOptionItem
    )
  ...
  focusNextOption() {
    ...
    const nextOptionItem =
      this.resultOptionItems[nextIndex] || this.resultOptionItems[0]
    ;(nextOptionItem as HTMLElement).focus()
  }

  get resultOptionItems() {
    return Array.from(
      this.resultsElement.querySelectorAll("[data-result-option-item]")
    )
  }
  ...
  get searchInput() {
    return (this._searchInput =
      this._searchInput || this.element.querySelector('input[type="search"]'))
  }
  ...
  get searchContext() {
    ...
    const selector = (this.element as HTMLElement).dataset
      .searchContextValueSelector
    if (selector)
      return (
        (this.element.parentNode.querySelector(selector) ||
          this.element.closest(selector)) as HTMLInputElement
      ).value
  ...
  get searchTermMinimumLength() {
    return (this.element as HTMLElement).dataset
      .pbTypeaheadKitSearchTermMinimumLength
  }

  get searchDebounceTimeout() {
    return (this.element as HTMLElement).dataset
      .pbTypeaheadKitSearchDebounceTimeout
  }

  get resultsElement() {
    return (this._resultsElement =
      this._resultsElement ||
      this.element.querySelector("[data-pb-typeahead-kit-results]"))
  }
  ...
  get resultsOptionCache() {
    return (this._resultsOptionCache = this._resultsOptionCache || new Map())
  }
