...
export default function Thumbnails({
  ...
  const shouldBeCentered =
    thumbnailWidth * urls.length < viewportSize.width * 0.9
  ...
  useLayoutEffect(() => {
    ...
    const thumbWidth = currentThumbnail.clientWidth,
      ...
      firstThumb: HTMLButtonElement = thumbnailsRef.current.querySelector(
        ".Thumbnail:first-child"
      ),
      lastThumb: HTMLButtonElement = thumbnailsRef.current.querySelector(
        ".Thumbnail:last-child"
      )

    firstThumb.style.marginLeft = `${
      thumbnailsRef.current?.clientWidth / 2 - thumbWidth / 2
    }px`
    lastThumb.style.marginRight = `${
      thumbnailsRef.current?.clientWidth / 2 - thumbWidth / 2
    }px`
  ...
  return (
    <div className={css} ref={thumbnailsRef}>
      {urls.map((url, i) => {
