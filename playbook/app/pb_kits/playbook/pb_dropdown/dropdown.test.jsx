...
test('external single-select event ignores disabled options', async () => {
  const onSelect = jest.fn()
  ...
  render(
    <Dropdown
        data={{ testid: testId, id: 'external-single' }}
        id="external-single"
        onSelect={onSelect}
  ...
  document.dispatchEvent(
    new CustomEvent('pb:dropdown:select', {
      detail: {
        dropdownId: 'external-single',
        optionId: 'disabled-id',
      },
    })
  )
...
test('external multi-select event ignores disabled options', async () => {
  const onSelect = jest.fn()
  ...
  render(
    <Dropdown
        data={{ testid: testId, id: 'external-multi' }}
        id="external-multi"
        multiSelect
        onSelect={onSelect}
  ...
  document.dispatchEvent(
    new CustomEvent('pb:dropdown:select', {
      detail: {
        dropdownId: 'external-multi',
        optionIds: ['disabled-id', 'enabled-id'],
      },
    })
  )
  ...
  const selectedPayload = onSelect.mock.calls[onSelect.mock.calls.length - 1][0]
  expect(selectedPayload).toHaveLength(1)
  expect(selectedPayload[0].id).toBe('enabled-id')
})
