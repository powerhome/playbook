...
test("generated default Trigger and Container when none passed in", () => {
  ...
  const defaultTrigger = kit.querySelector(
...
test("generated customDisplay for trigger", () => {
  render(
    <Dropdown data={{ testid: testId }}
        options={options}
    >
      <Dropdown.Trigger customDisplay={<Icon icon="flag" />} />
      {options.map((option) => (
        <Dropdown.Option key={option.id}
            option={option}
        />
...
test("generated placeholder prop", () => {
  render(
    <Dropdown data={{ testid: testId }}
        options={options}
    >
      <Dropdown.Trigger placeholder="Select a country" />
      {options.map((option) => (
        <Dropdown.Option key={option.id}
            option={option}
        />
...
test('placeholder shows default "Select..." when not provided', () => {
  render(<Dropdown data={{ testid: testId }}
      options={options}
         />);
...
test("MultiSelect prop to allow multiple selections + add correct Form Pills", () => {
  ...
  const formPills = kit.querySelectorAll(
    ".pb_form_pill_kit.pb_form_pill_primary",
  );
...
test("defaultValue works with multiSelect", () => {
  ...
  const kit = screen.getByTestId(testId);
  expect(
    kit.querySelectorAll(".pb_form_pill_kit.pb_form_pill_primary"),
  ).toHaveLength(2);
  const option2 = Array.from(kit.querySelectorAll(".pb_dropdown_option_list"));
  const firstOpt = options[0].label;
  expect(option2[0]).not.toHaveTextContent(firstOpt);
...
test("quickpick option values are Date objects", () => {
  ...
  const thisMonthOption = Array.from(options).find(
    (opt) => opt.textContent === "This Month",
  );
