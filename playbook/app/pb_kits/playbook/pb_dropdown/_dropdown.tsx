...
let Dropdown = (props: DropdownProps, ref: any): React.ReactElement | null => {
  ...
  const [isDropDownClosed, setIsDropDownClosed, toggleDropdown] =
    useDropdown(isClosed);
  ...
  const blankSelectionOption: GenericObject = blankSelection
    ? [{ label: blankSelection, value: "" }]
    : [];
  const optionsWithBlankSelection =
    blankSelectionOption.concat(dropdownOptions);
  ...
  const handleOptionClick = (clickedItem: GenericObject) => {
    if (multiSelect) {
      ...
      if (variant === "quickpick" && Array.isArray(clickedItem.value)) {
        ...
        if (controlsStartId) {
          const startPicker = (
            document.querySelector(`#${controlsStartId}`) as HTMLElement & {
              _flatpickr?: any;
            }
          )?._flatpickr;
        ...
        if (controlsEndId) {
          const endPicker = (
            document.querySelector(`#${controlsEndId}`) as HTMLElement & {
              _flatpickr?: any;
            }
          )?._flatpickr;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div
            className={`dropdown_wrapper ${error ? "error" : ""}`}
            onBlur={() => {
