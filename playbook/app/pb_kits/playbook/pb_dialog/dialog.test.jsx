...
test("renders the title and body text", async () => {
  ...
  await waitFor(() =>
    expect(queryByText("Header Title is the Title Prop")).toHaveTextContent(
      title
    )
  )

  await waitFor(() =>
    expect(queryByText("Hello Body Text, Nice to meet ya.")).toHaveTextContent(
      text
    )
  )
...
test("renders the buttons", async () => {
  ...
  await waitFor(() =>
    expect(queryByText("Okay")).toHaveTextContent(confirmButton)
  )

  await waitFor(() =>
    expect(queryByText("Cancel Button")).toHaveTextContent(cancelButton)
  )
