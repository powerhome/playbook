...
const Currency = (props: CurrencyProps) => {
  ...
  const getAmount = abbreviate
      ? getAbbreviatedValue("amount")
      : getMatchingDecimalAmount,
    ...
    <div {...ariaProps} {...dataProps} className={classes} id={id}>
      ...
      <div className={`pb_currency_wrapper${variantClass || emphasizedClass}`}>
        {unstyled ? (
          ...
          <>
            <Body className="dollar_sign" color="light" dark={dark}>
              {symbol}
            </Body>

            <Title className="pb_currency_value" dark={dark} size={sizes[size]}>
              {getAmount}
            </Title>

            <Body className="unit" color="light" dark={dark}>
              {getAbbreviation}
              {unit ? unit : getDecimalValue}
            </Body>
          </>
