...
import { BitValues, Binary, Display, DisplaySizes, None, Sizes } from "../types"
...
const getResponsivePropClasses = (
...
const PROP_CATEGORIES: {
  [key: string]: (props: { [key: string]: any }) => string
} = {
  ...
  flexDirectionProps: ({ flexDirection }: FlexDirection) => {
    if (typeof flexDirection == "object") {
      ...
      return flexDirection
        ? `flex_direction_${camelToSnakeCase(flexDirection)}`
        : ""
    }
  },
  ...
  justifyContentProps: ({ justifyContent }: JustifyContent) => {
    if (typeof justifyContent === "object") {
      ...
      return justifyContent
        ? `justify_content_${camelToSnakeCase(justifyContent)}`
        : ""
    }
  },
...
export const globalProps = (
  ...
  return Object.keys(PROP_CATEGORIES)
    .map(key => {
