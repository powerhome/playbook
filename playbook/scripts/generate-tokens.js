 ...
 *   or types/colors.ts), run this script and stage the updated colors.json — or the pre-commit hook
 *   will run it on commit and fail until you do.
 *
 ...
 *   then update .husky/pre-commit (VerifyTokens) and verify_tokens.sh to check each generated file.
 *
 * Usage (run from playbook/ directory):
 *   yarn generate:tokens
 *   node scripts/generate-tokens.js
 *
 * The pre-commit hook runs this script on commit when token SCSS or types/colors.ts are staged (VerifyTokens).
 */
...
if (Object.keys(colors).length === 0) {
  console.error(
    "❌ No accessible keys found in :export (check types/colors.ts)"
  )
  process.exit(1)
}
